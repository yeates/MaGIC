...
class LatentDiffusion(DDPM):
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False, **kwargs):
        ...
        #     return x_recon[0]
        # else:
        if kwargs.get('return_inter', False):
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, c_adm=None, **kwargs):
        if self.conditioning_key is None:
            ...
            out, inter_feats = self.diffusion_model(x, t, context=cc, **kwargs)
        elif self.conditioning_key == 'hybrid':
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out, inter_feats = self.diffusion_model(xc, t, context=cc, **kwargs)
